...
export function FileList() {
  ...
  function renderContent() {
    ...
    if (filesQuery.isError) {
      return (
        <ErrorState
          errorMessage={filesQuery.error.message || 'Unknown error'}
        />
      )
    }
          ...
          filesQuery.data?.length ? (
            <div className='flex items-center'>
              ...
              <Badge variant='primary' size='md'>
                {filesQuery.data.length}{' '}
                {filesQuery.data.length === 1 ? 'file' : 'files'}
              </Badge>
            </div>
          ) : null
        }
...
const LoadingState = memo(() => (
  <div className='py-8 text-center text-gray-500 animate-pulse flex flex-col items-center'>
    <svg
...
const ErrorState = memo(({ errorMessage }: { errorMessage: string }) => (
  <div className='py-6 text-center text-red-500 bg-red-50 rounded-lg border border-red-100'>
    <svg
      className='w-8 h-8 mx-auto mb-3 text-red-400'
      xmlns='http://www.w3.org/2000/svg'
      viewBox='0 0 24 24'
      fill='none'
      stroke='currentColor'
...
const FileRow = memo(({ file, onDelete, isDeleting }: FileRowProps) => (
  <FileCard
    fileName={file.name}
    fileSize={file.size}
